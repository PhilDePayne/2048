...
procBt(void* arg)
{
  ...
  uart1SendString((tU8 *)"+++");
  osSleep(50);
  uart1SendString((tU8 *)"ATI\r");
  osSleep(50);
  uart1SendString((tU8 *)"AT+BTLNM\r");
  
  osSleep(100);
  uart1SendString((tU8 *)"AT+BTBDA\r");
  osSleep(100);
  uart1SendString((tU8 *)"AT+BTINQ=5\r");
  osSleep(600);

  //Switch to server mode where other BT devices can detect this unit
  //during an 'inquiry'
  uart1SendString((tU8 *)"AT+BTSRV=1\r");
  ...
  while(1)
  {
    ...
    while (stopRecvProc == FALSE)
    {
      ...
      if (consolGetChar((char *) &rxChar) == TRUE)
      {
...
btInquiry(void)
{
  ...
  uart1SendString((tU8 *)"+++");
  osSleep(100);
  uart1SendString((tU8 *)"AT+BTINQ=6\r");
  ...
  while((done == FALSE) && ((ms - timeStamp) < 6500))
  {
    ...
    switch(cnt % 150)
    {
      case   0: lcdPuts((tU8 *)"   ");break;
      case  25: lcdPuts((tU8 *)".  ");break;
      case  50: lcdPuts((tU8 *)".. ");break;
      case  75: lcdPuts((tU8 *)"...");break;
      case 100: lcdPuts((tU8 *)" ..");break;
      case 125: lcdPuts((tU8 *)"  .");break;
      default: break;
    }
    cnt ++;
  }
  ...
  if (foundBtUnits[cursorPos].active == TRUE)
  {
    ...
    uart1SendString((tU8 *)"AT+BTSDP=");
    uart1SendString(foundBtUnits[cursorPos].btAddress);
    uart1SendString((tU8 *)"\r");
        ...
        if (rxChar == 0x0a)
        {
          ...
          else if (memcmp(recvBuf, "+BTSDP: ", 8) == 0)
          {
            ...
            if (numServices == 1)
            {
              ...
              //skip to first '"'
              pStr =(tU8 *) strchr((const char *) &recvBuf[10], '\"');
              if (pStr != NULL)
              {
                tU8 i;
    ...
    lcdGotoxy(2,16+(14*1));
    lcdPuts((tU8 *)" services");
    
    if (numServices > 0)
    {
      lcdGotoxy(2,16+(14*2));
      lcdPuts((tU8 *)"First service");
      lcdGotoxy(2,16+(14*3));
      lcdPuts((tU8 *)" name is:");
      lcdGotoxy(2,16+(14*4));
      lcdPuts(serviceName);
...
btSetAddress(void)
{
  ...
  lcdGotoxy(18,20);
  lcdColor(BT_BACKGROUND_COLOR,0xfd);
  lcdPuts((tU8 *)"Set address:");
  lcdGotoxy(6,34);
  lcdPuts((tU8 *)"Move cursor and");
  lcdGotoxy(6,48);
  lcdPuts((tU8 *)"set each char.");
  lcdGotoxy(6,62);
  lcdPuts((tU8 *)"Exit with c-key");
  ...
  while(done == FALSE)
  {
    ...
    if (anyKey != KEY_NOTHING)
    {
      ...
      if (anyKey == KEY_CENTER)
      {
        ...
        menu.pHeaderText = (tU8 *) "Exit";
        menu.headerTextXpos = 50;
        menu.pChoice[0] = (tU8 *) "Cancel";
        menu.pChoice[1] = (tU8 *) "Store new addr";
        menu.bgColor       = 0;
        ...
        switch(drawMenu(menu))
        {
          ...
          case 1:         //Store new address
            ...
            //*************************************************************
            uart1SendString((tU8 *)"AT+BTSET=1,");
  ...
  while(done == FALSE)
  {
    ...
    if (anyKey != KEY_NOTHING)
    {
      ...
      if (anyKey == KEY_CENTER)
      {
        ...
        menu.pChoice[0] = (tU8 *) "Cancel";
        menu.pChoice[1] = (tU8 *) "Store new name";
        menu.bgColor       = 0;
        menu.borderColor   = 0x6d;
        menu.headerColor   = 0;
        menu.choicesColor  = 0xfd;
        ...
        switch(drawMenu(menu))
        {
          ...
          case 1:         //Store new address
            ...
            //*************************************************************
            uart1SendString((tU8 *)"AT+BTFLS\r");
            break;
          default: break;
        }
        done = TRUE;
...
drawBtMenuCursor(tU8 cursor)
{
  ...
  for(row=0; row<=5; row++)
  {
    ...
    switch(row)
    {
      case 0: lcdPuts((tU8 *)"Inquiry"); break;
      case 1: lcdPuts((tU8 *)"Set name"); break;
      case 2: lcdPuts((tU8 *)"Set address"); break;
      case 3: if (btCommandMode == FALSE) lcdPuts((tU8 *)"Set comm.mode"); else lcdPuts((tU8 *)"Set data mode"); break;
      case 4: if (btSleepState == FALSE) lcdPuts((tU8 *)"Deactivate BT"); else lcdPuts((tU8 *)"Activate BT"); break;
      case 5: lcdPuts((tU8 *)"Main menu"); break;
      default: break;
    }
  }
}
...
drawBtMenu(void)
{
  ...
  if (btSleepState == FALSE)
    ...
    lcdPuts((tU8 *)"Inactive");

  lcdGotoxy(32,115);
  if (btCommandMode == FALSE)
    lcdPuts((tU8 *)"Data mode");
  else
    lcdPuts((tU8 *)"Command mode");
}
